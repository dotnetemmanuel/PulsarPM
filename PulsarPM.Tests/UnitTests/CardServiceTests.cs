...
public class CardServiceTests
{
  ...
  [Fact]
  public async Task EditCardStatus_ShouldReturnUpdatedStatus()
  {
    //Arrange
    ////Simulate Dropzones
    ...
    MudItemDropInfo<CardDTO>? mudItemDropInfo = new(card, "Backlog", 0);
    MudItemDropInfo<CardDTO>? mudItemDropInfo2 = new(null, "Done", 0);
    ...
    //Act
    //Satus change, card is moved to zone nr2.
    card.Status = mudItemDropInfo2.DropzoneIdentifier;
    var result = await _cardService.UpdateCardAsync(card);

    //Assert
    Assert.NotNull(result);
    Assert.Equal(card.Status, result.Status);
  }
}
